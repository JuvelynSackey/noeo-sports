...
Everything else in the roadmap (models, ensembling, calibration, drift
monitoring, auth, full dashboard) is **not yet implemented**. The schema for
most of it already exists (see `app/database/models/`) so later phases can
build on stable tables without re-migrating, but the business logic behind
those tables is still to come.
...
python scripts/sync_competitions.py   # runs discovery against the mock provider
...
```
app/
  api/            FastAPI app (Phase 1: health, competitions, sync — grows with later phases)
  data/
  ...
  services/
    ...
    competition_discovery.py orchestrates provider -> DB for competitions/seasons
...
scripts/          CLI entry points (init_db, sync_competitions)
tests/            pytest suite (provider contract, discovery, season detection, DB constraints)
```
...
2. Historical data ingestion + validation + team mapping
